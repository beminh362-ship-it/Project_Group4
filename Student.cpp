...
void Student::takeAttendance(string classId) {
    ...
    // 4. Check if already attended (Check file CSV trực tiếp hoặc danh sách trong RAM)
    // Cách 1: Dùng hàm có sẵn trong CSVHelper
    if (CSVHelper::hasStudentAttended(classId, studentId)) {
        cout << "You have already taken attendance for this session.\n";
        return;
    }

    // Cách 2: Check ngay trong vector activeSession (Cột 4)
